    ...
    <div className="container px-4 py-6 mx-auto max-w-7xl">
      <div className="flex items-center gap-2 mb-4">
        <Button variant="ghost" size="sm" asChild className="p-0">
          <Link to={category ? `/forum/category/${category.slug}` : "/forum"} className="flex items-center">
            <ChevronLeft className="h-4 w-4" />
            <span>Voltar para {category ? category.name : 'o Fórum'}</span>
          </Link>
        </Button>
      </div>
      ...
      <ForumLayout>
        {category ? (
          ...
          <div className="text-center py-10">
            ...
            <Button asChild>
              <Link to="/forum">Ver categorias</Link>
            </Button>
          </div>
        )}
      </ForumLayout>
