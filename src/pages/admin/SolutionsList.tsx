    ...
    const fetchSolutions = async () => {
      try {
        ...
        const transformedSolutions = (data || []).map(item => ({
          ...
          slug: item.title?.toLowerCase().replace(/\s+/g, '-') || '', // Generate slug from title
          ...
          expected_results: item.description || '', // Use description as fallback
          success_metrics: item.roi_potential || '', // Use roi_potential as fallback
          target_audience: item.description || '', // Use description as fallback
          prerequisites: item.description || '' // Use description as fallback
        }));
