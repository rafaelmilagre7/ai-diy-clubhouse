...
import { CheckCircle, XCircle, AlertCircle, RefreshCw, Settings, MessageSquare, Loader2, Bug } from 'lucide-react';
...
const WhatsAppConfigPanel = () => {
  ...
  const getStatusBadge = (isValid: boolean | undefined, error?: string) => {
    ...
    return isValid ? <Badge variant="default" className="bg-green-600">Válido</Badge> : <Badge variant="destructive">Inválido</Badge>;
  };
  return <div className="space-y-6">
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          {configData?.error ? <div className="flex items-center gap-2 p-3 bg-red-50 rounded-lg">
              <XCircle className="h-5 w-5 text-red-500" />
              <span className="text-red-700">Erro: {configData.error}</span>
            </div> : configData ? <div className="grid grid-cols-2 gap-4">
              <div className="flex items-center gap-2">
                {configData.hasToken ? <CheckCircle className="h-4 w-4 text-green-500" /> : <XCircle className="h-4 w-4 text-red-500" />}
                <span>WHATSAPP_API_TOKEN</span>
              </div>
              <div className="flex items-center gap-2">
                {configData.hasPhoneId ? <CheckCircle className="h-4 w-4 text-green-500" /> : <XCircle className="h-4 w-4 text-red-500" />}
                <span>WHATSAPP_PHONE_NUMBER_ID</span>
              </div>
              <div className="flex items-center gap-2">
                {configData.hasBusinessId ? <CheckCircle className="h-4 w-4 text-green-500" /> : <XCircle className="h-4 w-4 text-red-500" />}
                <span>WHATSAPP_BUSINESS_ID</span>
              </div>
              <div className="flex items-center gap-2">
                {configData.hasWebhookToken ? <CheckCircle className="h-4 w-4 text-green-500" /> : <XCircle className="h-4 w-4 text-red-500" />}
                <span>WHATSAPP_WEBHOOK_TOKEN</span>
              </div>
            </div> : <p className="text-muted-foreground">Clique em "Verificar" para checar a configuração</p>}
          
          <Button onClick={checkConfiguration} disabled={isCheckingConfig} className="w-full">
            {isCheckingConfig ? <>
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          {connectionData?.error ? <div className="flex items-center gap-2 p-3 bg-red-50 rounded-lg">
              <XCircle className="h-5 w-5 text-red-500" />
              <span className="text-red-700">Erro: {connectionData.error}</span>
            </div> : connectionData?.success ? <div className="space-y-2">
              <div className="flex items-center gap-2 p-3 bg-green-50 rounded-lg">
                <CheckCircle className="h-5 w-5 text-green-500" />
                <span className="text-green-700">Conexão bem-sucedida</span>
              </div>
              {connectionData.details && <div className="p-3 rounded-lg bg-gray-800">
            ...
            </div> : connectionData ? <div className="flex items-center gap-2 p-3 bg-yellow-50 rounded-lg">
              <AlertCircle className="h-5 w-5 text-yellow-500" />
              <span className="text-yellow-700">{connectionData.message}</span>
            </div> : <p className="text-muted-foreground">Clique em "Testar" para verificar a conexão</p>}
      ...
      <Card>
        ...
        <CardContent>
          <Button onClick={runFullDiagnosticHandler} disabled={isRunning} className="w-full" variant="outline">
            {isRunning ? <>
                <Loader2 className="h-4 w-4 mr-2 animate-spin" />
      ...
      <Card>
        ...
        <CardContent>
          <div className="bg-black text-green-400 p-4 rounded-lg font-mono text-sm max-h-64 overflow-y-auto">
            {logs.length === 0 ? <p className="text-gray-500">Nenhum log ainda...</p> : logs.map((log, index) => <div key={index} className="mb-1">
                  {log}
                </div>)}
          </div>
        </CardContent>
