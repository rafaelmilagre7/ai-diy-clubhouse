...
export const SupabaseErrorDiagnostics: React.FC = () => {
  ...
  const [connectionInfo, setConnectionInfo] = useState<ConnectionInfo>({ status: 'disconnected' });
  ...
  const runDiagnostics = async () => {
    ...
    try {
      ...
      try {
        ...
        setConnectionInfo({ status: 'error' });
      }
      ...
      try {
        ...
        const criticalTables = (securityData as any[]).filter(row => 
        ...
        results.push({
          ...
          message: criticalTables.length === 0 
            ? 'Todas as tabelas estão protegidas por RLS' 
            : `${criticalTables.length} tabelas sem proteção RLS`,
          details: { criticalCount: criticalTables.length, totalTables: securityData.length }
        });
      } catch (error: any) {
        results.push({
          category: 'Segurança RLS',
                    ...
                    <div>
                      ...
                      <p className="text-sm text-muted-foreground">
                        {connectionInfo.status === 'connected' ? 'Conectado' : 
                         connectionInfo.status === 'error' ? 'Erro na conexão' : 'Desconectado'}
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="space-y-2 max-h-64 overflow-y-auto">
                    {securityData.map((row, index) => (
                      <div key={index} className="flex items-center justify-between p-3 border rounded">
                        ...
                        <Badge variant={
                          row.security_status.includes('SEGURO') ? 'default' :
                          row.security_status.includes('RLS DESABILITADO') ? 'secondary' :
                          'destructive'
                        }>
                          {row.security_status}
                        </Badge>
