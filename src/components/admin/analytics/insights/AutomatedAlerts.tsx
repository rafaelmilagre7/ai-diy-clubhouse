  ...
  return <Card className="border-0 shadow-xl bg-white/80 backdrop-blur-sm">
      <CardHeader>
        <div className="flex items-center justify-between">
          <CardTitle className="flex items-center gap-2">
            ...
            Alertas Automáticos
            {unreadCount > 0 && <Badge className="bg-red-500 text-white text-xs">
                {unreadCount}
              </Badge>}
          </CardTitle>
          {criticalCount > 0 && <Badge className="bg-red-100 text-red-800 border-red-200">
      ...
      <CardContent className="space-y-4">
        {alerts.length === 0 ? <div className="text-center py-8">
            ...
            <p className="text-sm text-gray-500">Todas as métricas estão dentro dos parâmetros normais</p>
          </div> : alerts.map(alert => <div key={alert.id} className={cn("p-4 border rounded-lg transition-all", !alert.isRead && "border-l-4", alert.type === 'critical' && !alert.isRead && "border-l-red-500 bg-red-50/50", alert.type === 'warning' && !alert.isRead && "border-l-orange-500 bg-orange-50/50", alert.type === 'info' && !alert.isRead && "border-l-blue-500 bg-blue-50/50", alert.isRead && "border-gray-200 opacity-75")}>
              <div className="flex items-start justify-between mb-3">
                ...
                <div className="flex items-center gap-2">
                  ...
                  {!alert.isRead && <Button variant="ghost" size="sm" onClick={() => markAsRead(alert.id)} className="h-6 w-6 p-0">
                      <Eye className="h-3 w-3" />
                    </Button>}
                  <Button variant="ghost" size="sm" onClick={() => dismissAlert(alert.id)} className="h-6 w-6 p-0 text-gray-400 hover:text-red-500">
                    <X className="h-3 w-3" />
                  </Button>
                </div>
              </div>
              
              <p className="mb-3 text-zinc-50">{alert.description}</p>
              
              <div className="flex items-center justify-between">
                <div className="text-sm text-gray-600">
                  <span className="font-medium text-zinc-50">Valor atual:</span> {alert.currentValue} 
                  <span className="mx-2">|</span>
                  <span className="font-medium text-gray-50">Limite:</span> {alert.threshold}
                </div>
                
                {alert.actionRequired && <Badge variant="outline" className="text-xs border-orange-200 text-orange-700">
                    🔧 Ação Necessária
                  </Badge>}
              </div>
            </div>)}
