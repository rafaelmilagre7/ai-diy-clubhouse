
import React from 'react';
import { Card, CardContent } from '@/components/ui/card';
import { Badge } from '@/components/ui/badge';
import { cn } from '@/lib/utils';
import { LucideIcon, TrendingUp, TrendingDown } from 'lucide-react';

interface GlassStatsCardProps {
  title: string;
  value: string | number;
  icon: LucideIcon;
  trend?: {
    value: number;
    label: string;
    type: 'positive' | 'negative' | 'neutral';
  };
  colorScheme?: 'primary' | 'secondary' | 'success' | 'warning' | 'info';
  loading?: boolean;
  className?: string;
}

const colorSchemes = {
  primary: {
    iconBg: 'bg-[#00EAD9]/20',
    iconColor: 'text-[#00EAD9]',
    gradient: 'from-[#00EAD9]/20 to-transparent'
  },
  secondary: {
    iconBg: 'bg-purple-500/20',
    iconColor: 'text-purple-400',
    gradient: 'from-purple-500/20 to-transparent'
  },
  success: {
    iconBg: 'bg-green-500/20',
    iconColor: 'text-green-400',
    gradient: 'from-green-500/20 to-transparent'
  },
  warning: {
    iconBg: 'bg-orange-500/20',
    iconColor: 'text-orange-400',
    gradient: 'from-orange-500/20 to-transparent'
  },
  info: {
    iconBg: 'bg-blue-500/20',
    iconColor: 'text-blue-400',
    gradient: 'from-blue-500/20 to-transparent'
  }
};

export const GlassStatsCard: React.FC<GlassStatsCardProps> = ({
  title,
  value,
  icon: Icon,
  trend,
  colorScheme = 'primary',
  loading = false,
  className
}) => {
  const colors = colorSchemes[colorScheme];

  if (loading) {
    return (
      <Card className={cn(
        "border-gray-800/50 bg-[#151823] backdrop-blur-xl relative overflow-hidden",
        className
      )}>
        <div className="absolute inset-0 bg-gradient-to-br from-white/5 to-transparent" />
        <CardContent className="p-6 relative">
          <div className="animate-pulse space-y-4">
            <div className="flex items-center justify-between">
              <div className="space-y-2">
                <div className="h-4 bg-gray-700/50 rounded w-24"></div>
                <div className="h-8 bg-gray-700/50 rounded w-16"></div>
              </div>
              <div className="h-12 w-12 bg-gray-700/50 rounded-xl"></div>
            </div>
            {trend && (
              <div className="h-4 bg-gray-700/50 rounded w-20"></div>
            )}
          </div>
        </CardContent>
      </Card>
    );
  }

  return (
    <Card className={cn(
      "border-gray-800/50 bg-[#151823] backdrop-blur-xl relative overflow-hidden group hover:bg-[#1A1F2E] transition-all duration-300",
      className
    )}>
      {/* Gradient Background */}
      <div className={cn(
        "absolute inset-0 bg-gradient-to-br opacity-50",
        colors.gradient
      )} />
      <div className="absolute inset-0 bg-gradient-to-br from-white/5 to-transparent" />
      
      <CardContent className="p-6 relative">
        <div className="flex items-center justify-between mb-4">
          <div className="space-y-1">
            <p className="text-sm font-medium text-gray-400">
              {title}
            </p>
            <p className="text-3xl font-bold text-white">
              {typeof value === 'number' ? value.toLocaleString('pt-BR') : value}
            </p>
          </div>
          
          <div className={cn(
            "p-3 rounded-xl transition-all duration-300 group-hover:scale-110 relative",
            colors.iconBg
          )}>
            <Icon className={cn("h-6 w-6", colors.iconColor)} />
          </div>
        </div>

        {trend && (
          <div className="flex items-center space-x-2">
            {trend.type === 'positive' ? (
              <TrendingUp className="h-4 w-4 text-green-400" />
            ) : trend.type === 'negative' ? (
              <TrendingDown className="h-4 w-4 text-red-400" />
            ) : null}
            
            <Badge 
              variant="secondary"
              className={cn(
                "text-xs font-medium border-0 bg-opacity-80",
                trend.type === 'positive' && "bg-green-500/20 text-green-400",
                trend.type === 'negative' && "bg-red-500/20 text-red-400",
                trend.type === 'neutral' && "bg-gray-500/20 text-gray-400"
              )}
            >
              {trend.type !== 'neutral' && (trend.type === 'positive' ? '+' : '')}{Math.abs(trend.value).toFixed(1)}% {trend.label}
            </Badge>
          </div>
        )}
      </CardContent>
    </Card>
  );
};
