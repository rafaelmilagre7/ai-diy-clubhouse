...
import { Activity, AlertTriangle, CheckCircle, Clock, RefreshCw, Signal, TrendingUp, XCircle, Bell, BellOff } from 'lucide-react';
              ...
              <div className="">
                ...
                <div>
                  ...
                  <p className="text-sm opacity-75">
                    Sistema funcionando {metrics.systemHealth === 'healthy' ? 'normalmente' : metrics.systemHealth === 'warning' ? 'com avisos' : 'com problemas críticos'}
                  </p>
                </div>
              </div>
        ...
        <Card>
          <CardHeader>
            <div className="flex items-center justify-between">
              ...
              {alerts.length > 0 && <Button onClick={clearAlerts} variant="outline" size="sm">
          ...
          <CardContent>
            <ScrollArea className="h-64">
              {alerts.length === 0 ? <div className="flex items-center justify-center h-32 text-center">
                  <div className="text-muted-foreground">
                    <CheckCircle className="h-8 w-8 mx-auto mb-2" />
                    <p>Nenhum alerta</p>
                  </div>
                </div> : <div className="space-y-3">
                  {alerts.map(alert => <div key={alert.id} className={`p-3 rounded-lg border ${alert.acknowledged ? 'opacity-50' : ''} ${alert.type === 'error' ? 'bg-red-50 border-red-200' : alert.type === 'warning' ? 'bg-yellow-50 border-yellow-200' : 'bg-blue-50 border-blue-200'}`}>
                      <div className="flex items-start justify-between">
                        ...
                        {!alert.acknowledged && <Button onClick={() => acknowledgeAlert(alert.id)} variant="ghost" size="sm">
                            OK
                          </Button>}
                      </div>
                    </div>)}
