...
import { CheckCircle, AlertTriangle, RefreshCw, Mail, Shield, Globe, Clock, Zap, TestTube } from 'lucide-react';
...
export const ResendConfigValidator: React.FC = () => {
  ...
  const getStatusIcon = (status: boolean) => {
    return status ? <CheckCircle className="h-4 w-4 text-green-500" /> : <AlertTriangle className="h-4 w-4 text-red-500" />;
  };
  const getStatusBadge = (status: boolean, label: string) => {
    return <Badge variant={status ? "default" : "destructive"} className="text-xs">
        {status ? "✅" : "❌"} {label}
      </Badge>;
  };
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          <div className="flex gap-2">
            <Button onClick={validateConfiguration} disabled={isValidating} className="flex items-center gap-2">
              {isValidating ? <RefreshCw className="h-4 w-4 animate-spin" /> : <TestTube className="h-4 w-4" />}
              {isValidating ? 'Validando...' : 'Validar Configuração'}
            </Button>
          </div>

          {healthStatus && <div className="space-y-3 mt-4">
              <Separator />
              ...
              <div className="p-3 rounded text-sm space-y-1 bg-gray-800">
                ...
                {healthStatus.issues && healthStatus.issues.length > 0 && <div className="text-red-600">
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          <div className="space-y-2">
            ...
            <Input id="test-email" type="email" placeholder="seu-email@exemplo.com" value={testEmail} onChange={e => setTestEmail(e.target.value)} />
          </div>

          <Button onClick={sendTestEmail} disabled={isSendingTest || !testEmail} className="flex items-center gap-2 w-full">
            {isSendingTest ? <RefreshCw className="h-4 w-4 animate-spin" /> : <Mail className="h-4 w-4" />}
            {isSendingTest ? 'Enviando...' : 'Enviar Email de Teste'}
          </Button>

          {testResult && <div className="space-y-2 mt-4">
              <Separator />
              <div className="flex items-center gap-2">
              ...
              {testResult.success && testResult.emailId && <div className="p-2 rounded text-sm bg-gray-900">
                  <strong>ID do Email:</strong> {testResult.emailId}
                </div>}
              
              {!testResult.success && testResult.error && <div className="bg-red-50 p-2 rounded text-sm text-red-700">
      ...
      <Card className="bg-blue-50 border-blue-200">
        <CardContent className="pt-6">
          <div className="space-y-2 text-sm">
            <h4 className="font-medium text-gray-100">💡 Dicas de Configuração:</h4>
