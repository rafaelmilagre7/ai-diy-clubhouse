...
export function ModernDashboardHeader({ userName }: { userName: string }) {
  return (
    <div className={cn(
      ...
      "bg-gradient-to-br from-viverblue to-viverblue-light"
    )}>
      {/* Fundo visual */}
      <div className="absolute inset-0 z-0 opacity-80 pointer-events-none" aria-hidden>
        <svg className="w-full h-full" viewBox="0 0 800 280" fill="none">
          <ellipse cx="500" cy="120" rx="340" ry="130" fill="#6de2de" fillOpacity="0.10" />
          <ellipse cx="300" cy="160" rx="140" ry="50" fill="#0ABAB5" fillOpacity="0.13" />
        </svg>
      </div>
      {/* Conteúdo */}
      <div className="relative z-10 flex flex-col md:flex-row items-center md:items-end justify-between px-6 md:px-12 py-10">
        <div>
          <div className="flex items-center gap-2 mb-2">
            ...
            <span className="font-semibold text-white text-lg tracking-wide">VIVER DE IA Club</span>
          </div>
          <h1 className="text-3xl md:text-4xl font-black leading-tight text-white drop-shadow-xl">
        ...
        <div className="hidden md:block mt-6 md:mt-0">
          <img 
            ...
            className="h-24 w-24 rounded-2xl shadow-lg object-cover bg-white/60"
