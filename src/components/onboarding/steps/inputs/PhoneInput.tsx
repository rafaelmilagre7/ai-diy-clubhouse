  ...
  return <div className="space-y-2">
      ...
      <div className="flex gap-2">
        <div className="w-28">
          <Select value={ddi} onValueChange={onChangeDDI} disabled={disabled}>
            ...
            <SelectContent className="bg-[#1A1E2E] border-neutral-700">
              {commonDDIs.map(item => <SelectItem key={item.value} value={item.value}>
                  {item.label}
                </SelectItem>)}
            </SelectContent>
          </Select>
        ...
        <div className="flex-1">
          <Input id="phone" type="tel" value={value} disabled={disabled} onChange={e => onChange(e.target.value)} onBlur={onBlur} placeholder="(00) 00000-0000" className={error ? "border-red-400" : isValid ? "border-green-400" : ""} />
        </div>
      </div>
