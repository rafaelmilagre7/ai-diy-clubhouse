...
interface SocialButtonProps extends Omit<ButtonProps, 'variant'> {
  platform: 'whatsapp' | 'linkedin' | 'twitter' | 'email';
}
 ...
 * Cores específicas por plataforma, mantendo consistência visual
 */
  ...
  ({ platform, className, children, ...props }, ref) => {
    ...
    return (
      <Button
        ref={ref}
        className={cn(
          "justify-start h-12 text-white shadow-md hover:shadow-lg transition-all duration-200 hover:scale-105",
          style.bg,
          platform === 'email' && "border-2 border-border",
          className
        )}
        {...props}
      >
        {style.icon && <span className="mr-3">{style.icon}</span>}
        {children}
      </Button>
    );
  }
