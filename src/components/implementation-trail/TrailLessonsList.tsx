...
import { Book, ExternalLink, Calendar, Star, Clock, Bookmark } from "lucide-react";
  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
      {sortedLessons.map((lesson) => (
        <Card key={lesson.id} className="bg-[#151823] border-[#0ABAB5]/30 hover:border-[#0ABAB5]/60 transition-all">
          <CardHeader className="pb-2">
            <div className="flex justify-between items-start mb-1">
              ...
              <Badge variant="outline" className="bg-[#151823]">
                <Star className="h-3 w-3 mr-1 text-amber-400" /> Prioridade {lesson.priority || 1}
              </Badge>
            </div>
