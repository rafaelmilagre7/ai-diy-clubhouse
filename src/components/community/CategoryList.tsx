...
export const CategoryList: React.FC<CategoryListProps> = ({
  ...
  if (isLoading) {
    return <div className={`${compact ? 'space-y-3' : 'grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6'}`}>
        {Array.from({
        length: compact ? 3 : 6
  ...
  if (compact) {
    return <div className="space-y-3">
        ...
        {categories.length > 5 && <Button variant="outline" size="sm" className="w-full mt-4" onClick={() => navigate('/comunidade')}>
            Ver todas as categorias
          </Button>}
      </div>;
  }
  return <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
      {categories.map(category => <Card key={category.id} className="hover:shadow-lg hover:border-primary/20 transition-all duration-200 cursor-pointer group" onClick={() => handleCategoryClick(category)}>
          <CardContent className="p-6">
            <div className="flex flex-col gap-4">
              {/* Header com ícone e contador */}
              <div className="flex items-start justify-between">
                <div className="h-12 w-12 rounded-lg flex items-center justify-center text-2xl flex-shrink-0 group-hover:scale-105 transition-transform" style={{
