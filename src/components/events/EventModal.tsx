...
export const EventModal = ({
  ...
  return <Dialog open onOpenChange={() => onClose()}>
      <DialogContent className="max-w-md">
        ...
        {event.cover_image_url && <div className="relative">
            <img src={event.cover_image_url} alt={event.title} className="w-full h-48 object-cover rounded-md" />
        ...
        <div className="space-y-4">
          <div className="flex flex-col gap-3">
            ...
            {(event.location_link || event.physical_location) && <div className="flex items-start gap-2 text-sm">
                <MapPin className="h-4 w-4 text-viverblue mt-0.5" />
                <div className="flex flex-col">
                  {event.physical_location && <span className="text-gray-700">{event.physical_location}</span>}
                  {event.location_link && <a href={event.location_link} target="_blank" rel="noopener noreferrer" className="text-viverblue hover:underline flex items-center gap-1">
          ...
          {event.description && <div className="pt-2">
              <h3 className="text-sm font-medium mb-1 text-gray-600">Descrição</h3>
              <p className="text-sm whitespace-pre-line text-gray-100">{event.description}</p>
            </div>}

          <div className="flex justify-end gap-2 pt-2">
            <Button asChild className="bg-viverblue hover:bg-viverblue/90">
              <a href={generateGoogleCalendarLink()} target="_blank" rel="noopener noreferrer" className="flex items-center gap-1.5">
