...
const handler = async (req: Request): Promise<Response> => {
  console.log(`📧 [SEND-INVITE] Nova requisição: ${req.method}`);
  ...
  try {
    const { email, token, isResend = false }: SendInviteRequest = await req.json();
    
    console.log(`📨 [SEND-INVITE] Enviando para: ${email}, Token: ${token}, Reenvio: ${isResend}`);
    ...
    console.log(`✅ [SEND-INVITE] Email enviado com sucesso:`, emailResponse.data?.id);
...
console.log("📧 [SEND-INVITE] Edge Function carregada!");
serve(handler);
