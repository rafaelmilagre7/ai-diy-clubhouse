...
INSERT INTO public.audit_logs (
  ...
  'data_migration',
  'update_onboarding_state_city_structure',
  jsonb_build_object(
    'message', 'Migração para suporte a estado+cidade no onboarding',
    'affected_records', (
      SELECT COUNT(*) FROM public.onboarding_final 
      WHERE personal_info ? 'state' AND personal_info ? 'city'
    ),
    'migration_date', now()
  ),
  'info'
