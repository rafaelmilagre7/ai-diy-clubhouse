-- Corrigir problemas de segurança pendentes

-- 1. Corrigir views SECURITY DEFINER (convertendo para funções)
-- Substituir admin_analytics_overview view por função segura
...
-- 2. Corrigir triggers sem search_path (só os críticos para segurança)
...
CREATE OR REPLACE FUNCTION public.handle_updated_at_secure()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path TO 'public'
...
-- 3. Criar função para logs de auditoria segura
...
AS $$
BEGIN
  -- Função silenciosa para logs de auditoria
  ...
  EXCEPTION
    WHEN OTHERS THEN
      -- Falha silenciosamente para não quebrar operações
      NULL;
  END;
END;
$$;