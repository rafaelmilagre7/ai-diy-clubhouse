-- ========================================================================
-- CORREÇÕES CRÍTICAS DE SEGURANÇA - PATCH DE SEGURANÇA v1.0
-- ========================================================================
...
CREATE POLICY "secure_profiles_update_policy" ON public.profiles
  FOR UPDATE 
  USING (
    -- Condição 1: Usuário pode atualizar apenas seu próprio perfil (exceto role_id)
    (auth.uid() = id AND OLD.role_id = NEW.role_id)
    OR 
    -- Condição 2: Apenas admins podem alterar role_id
    (is_user_admin(auth.uid()) AND auth.uid() != id)
  )
  WITH CHECK (
    -- Check 1: Usuário não pode se autopromover para admin
    CASE 
      WHEN auth.uid() = id AND OLD.role_id != NEW.role_id THEN 
        FALSE -- Usuários não podem alterar próprio role_id
      ELSE 
        TRUE
    END
  );
...
ON public.audit_logs(event_type, user_id, created_at) 
WHERE event_type = 'role_change';

CREATE INDEX IF NOT EXISTS idx_audit_logs_security_events 
ON public.audit_logs(event_type, severity, created_at) 
