...
-- SISTEMA DE ANALYTICS ADMINISTRATIVO - VIEWS OTIMIZADAS
-- Baseado nos dados reais disponíveis no Supabase

-- 1. VIEW: Estatísticas consolidadas do sistema
...
SELECT 
  ...
  -- Tempo médio de implementação (em minutos)
  COALESCE((
    SELECT AVG(EXTRACT(EPOCH FROM (completed_at - created_at))/60)::integer
    FROM public.progress 
    WHERE is_completed = true AND completed_at IS NOT NULL
  ), 0) as avg_implementation_time_minutes;

-- 2. VIEW: Dados de engajamento de usuários
CREATE OR REPLACE VIEW public.user_engagement_metrics AS
SELECT 
  DATE_TRUNC('day', p.created_at) as date,
  COUNT(*) as new_users,
  COUNT(*) FILTER (WHERE pr.role = 'admin') as new_admins,
  COUNT(*) FILTER (WHERE pr.role = 'member') as new_members,
  COUNT(*) FILTER (WHERE ur.name = 'formacao') as new_formacao_users
...
SELECT 
  ...
  COALESCE(AVG(EXTRACT(EPOCH FROM (pr.completed_at - pr.created_at))/60) FILTER (WHERE pr.is_completed = true), 0)::integer as avg_completion_time_minutes
...
SELECT 
  ...
  COUNT(DISTINCT lp.user_id) FILTER (WHERE lp.progress_percentage = 100) as users_with_completed_lessons,
...
SELECT 
  COALESCE(ur.name, p.role, 'member') as role_name,
  COUNT(*) as user_count,
  ROUND(COUNT(*)::numeric / (SELECT COUNT(*) FROM public.profiles)::numeric * 100, 2) as percentage
FROM public.profiles p
LEFT JOIN public.user_roles ur ON p.role_id = ur.id
GROUP BY COALESCE(ur.name, p.role, 'member')
ORDER BY user_count DESC;

-- Log da criação das views
INSERT INTO public.audit_logs (
  ...
  jsonb_build_object(
    ...
    'purpose', 'Sistema de analytics administrativo funcional baseado em dados reais',
    'created_at', NOW()
  )
);
