-- CORREÇÃO URGENTE DE POLÍTICAS RLS - FASE 1: TABELAS CRÍTICAS (CORRIGIDA)
...
DROP POLICY IF EXISTS "learning_courses_unified_access" ON public.learning_courses;
DROP POLICY IF EXISTS "learning_courses_admin_management" ON public.learning_courses;
...
INSERT INTO public.audit_logs (
  ...
  'rls_critical_fix',
  'fix_critical_rls_policies_phase_1_corrected',
  jsonb_build_object(
    'phase', '1 - Critical Tables RLS Fix (Corrected)',
    'tables_fixed', ARRAY[
      'forum_posts - Added missing SELECT, UPDATE, DELETE policies',
      'learning_courses - Consolidated conflicting policies into 2 clear ones',
      'profiles - Cleaned up and reduced to 3 essential policies',
      'user_roles - Removed anonymous access, admin-only access'
    ],
