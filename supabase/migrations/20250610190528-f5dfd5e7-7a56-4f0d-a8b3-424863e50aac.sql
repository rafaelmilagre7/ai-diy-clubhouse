...
-- Criar funções RPC necessárias para validação e sincronização de roles

-- Função para validar inconsistências nos roles dos usuários
...
BEGIN
  ...
  SELECT 
    ...
    ur.name as expected_role_name,
    ur.id as expected_role_id,
    CASE 
      ...
      WHEN p.role IS NOT NULL AND p.role_id IS NOT NULL AND p.role != ur.name THEN 'role_mismatch'
      ELSE 'unknown'
    END as issue_type
  ...
  WHERE 
    ...
    (p.role IS NOT NULL AND p.role_id IS NOT NULL AND p.role != ur.name)
  ORDER BY p.created_at DESC;
END;
$$;

-- Função para auditoria geral do sistema de roles
...
-- Função para sincronizar roles automaticamente
...
-- Função global is_admin() que usa o usuário atual
