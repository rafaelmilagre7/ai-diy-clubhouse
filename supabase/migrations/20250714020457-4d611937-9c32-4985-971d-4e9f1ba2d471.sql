-- Primeiro remover função conflitante se existir
DROP FUNCTION IF EXISTS public.sync_onboarding_to_profile() CASCADE;

-- Remover triggers conflitantes se existirem  
DROP TRIGGER IF EXISTS sync_onboarding_to_profile ON public.onboarding;
...
-- 3. Criar nova função para sincronização automática
...
-- 4. Aplicar novo trigger
CREATE TRIGGER onboarding_sync_trigger
