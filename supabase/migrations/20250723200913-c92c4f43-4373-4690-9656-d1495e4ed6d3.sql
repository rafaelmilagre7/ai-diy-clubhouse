-- Corrigir a lógica do current_step na função update_onboarding_step
...
BEGIN
  ...
  -- Calcular próximo step corretamente
  -- Se o step sendo completado é o mais alto completado, avançar para o próximo
  -- Caso contrário, manter o current_step atual se for maior
  next_step := GREATEST(
    p_step + 1,  -- Próximo step após o que está sendo salvo
    (SELECT COALESCE(MAX(s), 0) FROM unnest(updated_steps) AS s) + 1,  -- Próximo após o maior completado
    onboarding_record.current_step  -- Manter se já estiver em step mais avançado
  );
  
  -- Garantir que não ultrapasse o total de steps (6)
  next_step := LEAST(next_step, 6);
