-- 🔒 CORREÇÃO CRÍTICA: Proteção de Conteúdo Educacional Premium
...
-- ==========================================
-- 3. LEARNING COURSES - Reforçar proteção
-- ==========================================

-- A política existente já usa can_access_course_enhanced que está correta
-- Mas vamos garantir que não há brechas removendo políticas duplicadas se existirem

-- Verificar se há política que permite acesso sem verificação de enrollment
-- (mantendo apenas a política segura existente)

-- ==========================================
-- 4. PROTEÇÃO ADICIONAL - USER COURSE ACCESS
-- ==========================================
...
CREATE POLICY IF NOT EXISTS "users_can_view_own_course_access" ON public.user_course_access
FOR SELECT TO authenticated
USING (user_id = auth.uid() OR is_user_admin_secure(auth.uid()));

-- Política para inserção (apenas admins podem conceder acesso)
CREATE POLICY IF NOT EXISTS "admins_can_grant_course_access" ON public.user_course_access
FOR INSERT TO authenticated  
WITH CHECK (is_user_admin_secure(auth.uid()));

-- Política para atualização (apenas admins)
CREATE POLICY IF NOT EXISTS "admins_can_modify_course_access" ON public.user_course_access
