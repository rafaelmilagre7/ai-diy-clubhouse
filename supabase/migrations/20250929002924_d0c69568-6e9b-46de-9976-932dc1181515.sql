...
-- associados à organização do Mikael, não team members
UPDATE profiles 
SET is_master_user = false
WHERE email IN (
...
-- 3. Garantir que estes usuários tenham organization_id correto
-- (associados à organização do Mikael)
UPDATE profiles 
SET organization_id = (
...
INSERT INTO audit_logs (
  ...
  jsonb_build_object(
    'description', 'Corrigida estrutura de usuários da planilha',
    'master_users', 1,
    'individual_users_associated', 16,
    'organization_name', 'Formação Vendas e Gestão',
    'change', 'Usuários da coluna email agora são usuários individuais associados ao master da coluna acesso_vinculado_a'
  ),
  'info'
);