-- Criar view admin_analytics_overview para estatísticas gerais
...
SELECT 
  ...
  (SELECT COUNT(*) FROM solutions WHERE status = 'published') as total_solutions,
  (SELECT COUNT(*) FROM progress WHERE is_completed = true) as completed_implementations,
  (SELECT ROUND(AVG(EXTRACT(EPOCH FROM (completed_at - created_at))/3600), 2) FROM progress WHERE is_completed = true AND completed_at IS NOT NULL) as avg_implementation_time_hours,
  (SELECT ROUND(
    (COUNT(*) FILTER (WHERE is_completed = true)::float / NULLIF(COUNT(*), 0)) * 100, 2
...
SELECT 
  ...
  ROUND(
    (COUNT(p.id) FILTER (WHERE p.is_completed = true)::float / NULLIF(COUNT(p.id), 0)) * 100, 2
  ) as completion_rate,
  ROUND(AVG(EXTRACT(EPOCH FROM (p.completed_at - p.created_at))/3600), 2) as avg_completion_time_hours
FROM solutions s
...
SELECT 
  ...
  ROUND((COUNT(p.id)::float / (SELECT COUNT(*) FROM profiles)) * 100, 2) as percentage
FROM user_roles ur
