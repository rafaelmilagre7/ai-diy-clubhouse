...
-- Fazer backup antes de remover (apenas se a tabela ainda existir)
DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'onboarding_simple') THEN
    ...
    RAISE NOTICE 'Backup de onboarding_simple criado';
  END IF;
END $$;
