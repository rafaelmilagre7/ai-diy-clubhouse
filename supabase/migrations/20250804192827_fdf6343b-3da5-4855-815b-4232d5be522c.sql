-- Remover função existente e criar uma nova
DROP FUNCTION IF EXISTS public.generate_event_recurrence(uuid);

-- Criar função correta para gerar eventos recorrentes
CREATE FUNCTION public.generate_event_recurrence(input_event_id uuid)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
