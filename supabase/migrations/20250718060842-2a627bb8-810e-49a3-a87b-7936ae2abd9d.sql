...
-- Fase 1: Correção de Tipos de Dados e Eliminação de Referências Incorretas

-- 1. Corrigir função get_user_profile_optimized com tipos corretos
...
-- 2. Corrigir função get_cached_profile com tipos corretos
...
-- Fase 2: Eliminar funções problemáticas que referenciam 'users' incorretamente
-- Remover funções que causam "permission denied for table users"

-- Função para verificar se é admin sem referenciar auth.users diretamente
...
-- Fase 3: Triggers para Sincronização Automática

-- Trigger para atualizar cache quando perfil for modificado
...
-- Trigger para invalidar cache quando roles forem modificados
...
-- Fase 4: Função de validação de integridade
...
COMMENT ON FUNCTION public.get_user_profile_optimized(uuid) IS 'Função otimizada para buscar perfil do usuário com tipos corretos';
COMMENT ON FUNCTION public.get_cached_profile(uuid) IS 'Função de cache de perfil com sincronização automática';
COMMENT ON FUNCTION public.current_user_is_admin() IS 'Verificação de admin sem referência direta a auth.users';
COMMENT ON FUNCTION public.invalidate_profile_cache() IS 'Trigger para invalidação automática de cache';
COMMENT ON FUNCTION public.cleanup_expired_cache() IS 'Limpeza automática de cache expirado';
COMMENT ON FUNCTION public.validate_auth_integrity() IS 'Validação de integridade do sistema de autenticação';