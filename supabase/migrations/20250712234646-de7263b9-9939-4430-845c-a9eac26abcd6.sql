-- MELHORIAS DE SEGURANÇA: Funções adicionais e triggers
...
-- 7. Atualizar função existente is_user_admin para maior segurança
...
BEGIN
  ...
  SELECT EXISTS(
    ...
    WHERE p.id = COALESCE(user_id, auth.uid())
      AND ur.name = 'admin'
      AND ur.is_system = true -- Apenas papéis de sistema
  ) INTO is_admin_user;
...
INSERT INTO public.audit_logs (
  ...
  jsonb_build_object(
    ...
    'improvements', ARRAY[
      ...
      'Função is_user_admin aprimorada'
    ],
    'timestamp', now()
  ),
  'high'
