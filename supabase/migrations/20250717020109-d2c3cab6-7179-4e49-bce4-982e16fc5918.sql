-- CRIAR FUNÇÃO is_user_admin FALTANTE + CORREÇÃO FINAL
-- ==================================================================
...
-- 3. Forçar correção de todos role_id NULL (caso ainda existam)
...
-- 4. Se não existir role padrão, criar um
...
-- 5. Garantir que TODOS os perfis tenham role válido
UPDATE public.profiles 
SET role_id = (
  SELECT id FROM public.user_roles 
  WHERE name IN ('member', 'membro', 'membro_club')
...
-- 6. Inserir onboarding para TODOS os usuários órfãos restantes
